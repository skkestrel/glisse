...
namespace sr
{
	namespace wh
	{
		...
		struct MVSKernel
		{
			...
			MVSKernel(const DevicePlanetPhaseSpace &planets, const Dvf64_3 &h0_log, const Dvf64 &_planet_rh, double _outer_bound, uint32_t _tbsize, float64_t _dt, uint32_t _maxkep) : planet_m(planets.m.data().get()),
																																													   mu(planets.m[0]),
			...
			__host__ __device__ static void kepeq(double dM, double ecosEo, double esinEo, double *dE, double *sindE, double *cosdE, uint16_t &flags, uint32_t maxkep)
			{
				double f, fp, delta;
			...
			__host__ __device__ static void drift(f64_3 &r, f64_3 &v, uint16_t &flags, double dt, double mu, uint32_t maxkep)
			{
			...
			__host__ __device__ static void step_forward(f64_3 &r, f64_3 &v, uint16_t &flags, f64_3 &a, uint32_t &deathtime_index, uint32_t _tbsize,
		...
		__global__ void MVSKernel_(f64_3 *r, f64_3 *v, uint16_t *flags, f64_3 *a, uint32_t *deathtime_index,
		...
		void WHCudaIntegrator::integrate_particles_timeblock_cuda(cudaStream_t stream, size_t planet_data_id, const DevicePlanetPhaseSpace &pl, DeviceParticlePhaseSpace &pa)
		{
			...
			MVSKernel_<<<grid_size, block_size, shared_mem, stream>>>(pa.r.data().get(), pa.v.data().get(), pa.deathflags.data().get(), device_particle_a.data().get(), pa.deathtime_index.data().get(),
